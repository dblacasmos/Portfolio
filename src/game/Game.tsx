...
const Game: React.FC = () => {
  ...
  useEffect(() => {
    return () => {
      ...
      // Cierra KTX2 (si la implementación lo soporta)
  ...
  // Reacciona al cierre del DestroyDroneCard #5
  useEffect(() => {
    const onDDCClose = (e: Event) => {
      const idx = (e as CustomEvent).detail?.index;
      if (idx === 5 && allDronesDown) {
        // Mostramos la EndDoor en cuanto cierre la última tarjeta.
        // (El vídeo arrancará con delay dentro de EndDoor.tsx)
      ...
      <div id="fs-root" ref={fsRootRef} style={{ position: "relative", width: "100%", height: "100%" }}>
        ...
        <Canvas
          ...
          onCreated={({ gl }) => {
            ...
            try { ktx2HandleRef.current = initKTX2Loader(r, (CFG as any)?.decoders?.basisPath ?? "/assets/basis/") ?? null; } catch { }
            glRef.current = r;
