...
export function tuneMaterials(root: THREE.Object3D) {
  ...
  root.traverse((obj) => {
    ...
    const applyTex = (t?: THREE.Texture) => {
      ...
      // Política de mipmaps: desactivar en texturas pequeñas para ahorrar VRAM.
      // Para texturas comprimidas con mipmaps embebidos, no forzamos generateMipmaps.
      const isCompressed =
        (t as any).isCompressedTexture === true
      ...
      if (!isCompressed) {
        ...
        // minFilter: sube a mipmap si estaba en Nearest
        if (t.minFilter === THREE.NearestFilter) {
