...
T-9 “Gólem”, Buena cacería.`;
    ...
    return (
        <OverlayShell
            ...
            contentClassName="flex items-start justify-center pt-6 sm:pt-10"
            contentStyle={{ pointerEvents: "auto" }}
        >
            <div
                className="relative w-[min(1100px,96vw)] rounded-2xl border border-cyan-400/30 bg-[rgba(10,16,24,.88)] shadow-[0_0_40px_rgba(56,189,248,.18)_inset] overflow-hidden"
                role="dialog"
                aria-modal="true"
                aria-label={title}
                /* Altura fluida sin scroll interno */
                style={{
                    /* La tarjeta nunca excede la ventana, dejando respiración para paddings */
                    height: "min(92svh, 920px)",
                    /* Altura de paneles laterales (izq/dcha) adaptable sin scrollbars
                     * 240px en móvil, hasta ~560px en pantallas grandes
                     * usada por los paneles con h: var(--pane-h)*/
                    ["--pane-h" as any]: "clamp(240px, 36vh, 560px)",
                }}
            >
                {/* Header */}
                <div className="px-4 sm:px-5 pt-4 pb-2 flex items-center justify-between gap-3">
                    ...
                    <div className="flex items-center gap-2">
                        <div
                            className={`text-[11px] px-2 py-1 rounded-md ring-1 ${speaking
                        ...
                        {/* En móvil cabe justo encima; se oculta si sólo hay 1 voz */}
                ...
                {/* Grid responsiva: 1 col en móvil, 3 áreas en ≥ md */}
                <div className="grid grid-cols-1 md:grid-cols-[minmax(260px,360px)_1fr_minmax(240px,360px)] gap-4 px-4 sm:px-5 pb-5">
                    {/* IZQUIERDA: avatar */}
                    <div className="h-[var(--pane-h)] rounded-xl overflow-hidden border border-white/10 bg-black relative">
                        <video
                            ref={videoRef}
                    ...
                    {/* CENTRO: texto y CTA (sin scroll interno) */}
                    <div className="grid grid-rows-[auto_1fr_auto] gap-3 min-h-0">
                        <div className="rounded-xl bg-black/30 border border-white/10 p-3 flex items-center gap-3">
                            ...
                            <div className="text-cyan-200/90 text-sm">
                                OPERADOR — Charly Gepeto
                            </div>
                        </div>

                        {/* NOTA: sin max-height ni overflow-auto */}
                        <div className="rounded-xl bg-black/25 border border-white/10 p-4 min-h-[120px]">
                            <div className="text-cyan-200/90 text-[12px] sm:text-[13px] leading-relaxed whitespace-pre-wrap">
                                {typed}
                    ...
                    {/* DERECHA: dron (zoom rueda, sin scrollbars) */}
                    <div
                        className="h-[var(--pane-h)] rounded-xl overflow-hidden border border-white/10 bg-[radial-gradient(120%_120%_at_50%_15%,rgba(56,189,248,.12),transparent_60%)] relative"
                        ref={zoomAreaRef}
                        onWheelCapture={onWheelZoom}
                        style={{ overscrollBehavior: "contain", touchAction: "none" }}
                    >
                        <div className="absolute top-2 left-2 text-[10px] tracking-widest text-cyan-200/90">
                            SCROLL para aumentar o disminuir
                        </div>
                        <Canvas
                            dpr={[1, 1.5]}
