/*  ========================================
    FILE: src/game/overlays/OverlayShell.tsx
    ======================================== */
...
type OverlayShellProps = {
    ...
    onClose: () => void;                                    // Cierre solicitado por backdrop/tecla/tap (o desde botón del hijo)
    ariaLabel?: string;                                     // Para accesibilidad del diálogo
    zIndex?: number;                                        // Z-index del overlay (por defecto 2147483647)

    keyboardKey?: KeyboardEvent["key"] | null;              // Manejador de tecla para cerrar (por defecto "Enter"); null = desactivar
    closeOnBackdrop?: boolean;                              // Cerrar con click en backdrop (true por defecto)
    pauseMusic?: boolean;                                   // Apaga música mientras está visible (default: false)
    squelchMenuEsc?: boolean;                               // Mientras visible, evita que ESC del menú haga nada (default: false)
    pointerLockSelector?: string;                           // Al cerrar, intenta recuperar pointer lock en este target (CSS selector)
    getPointerLockTarget?: () => HTMLElement | null;        // Alternativa al selector: callback para resolver el elemento objetivo       
    ...
    children: React.ReactNode;                              // Contenido del overlay (la “tarjeta”/UI real)
};
